...
void printmap(std::map<K, T> &tmp)
{
	std::cout << "map size: " << tmp.size() << std::endl;
	std::cout << "map contains: " << std::endl;
	typename std::map<K, T>::iterator it = tmp.begin();
	while (it != tmp.end())
	{
...
{
	std::vector <int> d;
	std::vector <int> d1(10, 2);
	std::vector <int> d2(d1);
	d2[0] = 1;
	d2[3] = 4;
	std::vector <int> d3(d2.begin(), d2.end());
	d3[1] = 7;
	d3[6] = 8;	
...
void map_test(){
	...
	m2.insert(std::make_pair(35, 27));
	printmap(m2);
	std::cout << "\tInsert {35,23}" << std::endl;
	m2.insert(std::make_pair(35, 23));
	printmap(m2);
